...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session_id) return jsonCors(req, { ok: false, error: 'missing session_id' }, { status: 400 });
    ...
    // Récupère ce que le webhook aura posé
    ...
    return jsonCors(req, {
      ...
      printful_order_id,
      store_id,
      // renvoie un tout petit debug (pas tout l’objet)
      session: { id: session.id, amount_total: session.amount_total, currency: session.currency },
    });
  } catch (e: any) {
